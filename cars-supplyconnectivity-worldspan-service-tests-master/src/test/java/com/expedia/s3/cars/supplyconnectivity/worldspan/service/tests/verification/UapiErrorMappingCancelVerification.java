...
import com.expedia.s3.cars.supplyconnectivity.messages.cancel.defn.v4.ErrorCollectionType;
 ...
 */
public class UapiErrorMappingCancelVerification {
    public static void errorMappingVerifier(ErrorCollectionType errorCollection,
        ...
        if (CompareUtil.isObjEmpty(errorList)) {
            ...
            switch (testDataErrHandle.getErrorType()) {
                ...
                case "ReferenceInvalidError":
                    if (CompareUtil.isObjEmpty(errorCollection.getReferenceInvalidErrorList())) {
                        Assert.fail("The actual ReferenceInvalidErrorList is null, not equal the expected.");
                    } else {
                        actErrorMessage = errorCollection.getReferenceInvalidErrorList().getReferenceInvalidError().get(0).getDescriptionRawText();
                        actErrorXpath = errorCollection.getReferenceInvalidErrorList().getReferenceInvalidError().get(0).getFieldKey().getXPath();
                    }
                    break;
                ...
                case "ReferenceUnavailableError":
                    if (CompareUtil.isObjEmpty(errorCollection.getReferenceUnavailableErrorList().getReferenceUnavailableError())) {
                        ...
                        actErrorXpath = errorCollection.getReferenceUnavailableErrorList().getReferenceUnavailableError().get(0).getFieldKey().getXPath();
                    }
                    break;
            ...
            if (!((CompareUtil.isObjEmpty(testDataErrHandle.getxPath()) && CompareUtil.isObjEmpty(actErrorXpath))
                    || actErrorXpath.contains(testDataErrHandle.getxPath()))) {
