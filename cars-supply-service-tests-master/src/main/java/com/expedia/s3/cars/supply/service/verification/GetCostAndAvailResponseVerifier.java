...
public class GetCostAndAvailResponseVerifier implements IVerification {
    ...
    public static void verifyReturn(CarSupplyGetCostAndAvailabilityResponseType response, TestScenario scenarios) {
        //verifyReturn car product returned
        final StringBuilder errorMsg = new StringBuilder();
        if (null == response) {
            errorMsg.append("No data return in GetCostAndAvail response.");
        }
        if (null == response.getCarProductList() || response.getCarProductList().getCarProduct() == null || response.getCarProductList().getCarProduct().isEmpty()) {
            errorMsg.append("No CarProduct return in GetCostAndAvail response.");
        }
